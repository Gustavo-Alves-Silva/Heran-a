...
public class CadastroIndividuo extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					CadastroIndividuo frame = new CadastroIndividuo(null);
	 ...
	 * Create the frame.
	 */
	public CadastroIndividuo(Especie especie) {
		especieG = especie;
//		System.out.println(genero.getFamilia());
//		System.out.println(genero.getFamilia().getOrdem());
//		System.out.println(genero.getFamilia().getOrdem().getClasse());
//		System.out.println(genero.getFamilia().getOrdem().getClasse().getFilo());
//		System.out.println(genero.getFamilia().getOrdem().getClasse().getFilo().getReino());
...
//		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		...
		btnCadastrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) { 
				String s = especie.getGenero().getFamilia().getOrdem().getClasse().getFilo().getReino().getNomeReino();


					
					String nomeGenero = txtCadastro.getText();

				Individuo individuo = new Individuo(s,
						especie.getGenero().getFamilia().getOrdem().getClasse().getFilo().getNomeFilo(),
						especie.getGenero().getFamilia().getOrdem().getClasse().getNomeClasse(),
						especie.getGenero().getFamilia().getOrdem().getNomeOrdem(), 
						especie.getGenero().getFamilia().getNomeFamilia(),
						especie.getGenero().getNomeGereno(),
						especie.getNomeEspecie(),
						nomeGenero);
				
					especie.getIndividuos().add(individuo);
					TelaIndividuoListar A = new TelaIndividuoListar(especie);
					System.out.println("Individuo Cadastrado");
					A.setVisible(true);
					setVisible(false);
