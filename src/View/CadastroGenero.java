...
public class CadastroGenero extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					CadastroGenero frame = new CadastroGenero(null);
	 ...
	 * Create the frame.
	 */
	public CadastroGenero(Familia familia) {
		familiaG = familia;
...
//		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		...
		btnCadastrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) { 
				String s = familia.getOrdem().getClasse().getFilo().getReino().getNomeReino();


					
					String nomeFamilia = txtCadastro.getText();

				Genero genero = new Genero(s,
						familia.getOrdem().getClasse().getFilo().getNomeFilo(),
						familia.getOrdem().getClasse().getNomeClasse(),
						familia.getOrdem().getNomeOrdem(), 
						familia.getNomeFamilia(),
						nomeFamilia);
					
					
					familia.getGeneros().add(genero);
					TelaGeneroListar A = new TelaGeneroListar(familia);
